          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {jenis === 'Pemasukan' ? (
                  <div className="space-y-1">
                    ...
                    <select 
                      className="flex h-9 w-full rounded-xl border border-white/5 bg-slate-900/80 text-xs text-white px-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
                      value={sumberDana}
            ...
            <div className="flex gap-2 mt-2">
              ...
              <Button 
                type="submit" 
                disabled={submitting} 
                className={`${editingId ? 'flex-1' : 'w-full'} bg-[#00e5a3] hover:bg-[#00cfa2] text-slate-950 font-extrabold text-xs h-9 rounded-xl transition-all shadow-md`}
              >
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <Input 
                ...
                className="pl-9 pr-8 bg-[#121829]/60 border-white/5 text-xs text-white h-10 rounded-xl w-full focus-visible:ring-emerald-500"
              />
              {searchTerm && (
                <button 
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-[#121829]/30 p-1.5 rounded-xl border border-white/5">
            <div className="grid grid-cols-3 gap-1 w-full sm:w-auto">
              {[
                ...
                { label: 'Pemasukan', value: 'Pemasukan', count: countPemasukan, color: 'text-emerald-400' },
                ...
                >
                  <span className={jenisFilter === item.value ? 'text-slate-950 hover:text-slate-950' : item.color}>{item.label}</span>
            ...
            <div className="text-[10px] text-slate-400 px-1 py-1 sm:py-0 font-medium text-right sm:text-left">
              Menampilkan <span className="text-emerald-400 font-bold">{filteredKas.length}</span> dari <span className="text-white">{kas.length}</span> transaksi
            </div>
          </div>

          {/* Collapsible Advanced Filters Row */}
          {showAdvancedFilters && (
            <div className="p-3 bg-[#121829]/80 border border-emerald-500/10 rounded-xl space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
              {/* Quick Date Presets */}
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-all ${
                        quickDate === preset.value
                          ? 'bg-emerald-500/15 border border-emerald-500/35 text-emerald-400 font-bold'
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    className="flex h-8 w-full rounded-lg border border-white/5 bg-slate-950/80 text-[11px] text-slate-300 px-2 focus:outline-none focus:ring-1 focus:ring-emerald-500"
